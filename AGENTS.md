...
- 对于边界收口、模块拆分、OpenSpec 归档这类连续架构收口操作，Agent 应自行阅读源码与文档、选择下一步并执行到待验收状态，不要停下来让用户决定下一块做什么；但不得跳过用户验收直接提交。
- 用户提出需求后，Agent 默认执行到可验收状态并汇报改动、验证结果和待验收事项；只有在用户明确验收通过或明确要求提交后，才允许执行 `git commit`。
- 仅在以下情况打断用户：
  - 涉及破坏性操作；
  - 可能导致数据丢失；
  - 需要用户提供缺失信息；
...
- 三端入口文件必须保持薄入口：
  - `apps/web-console/src/main.tsx` 只负责挂载 React 应用；
  - `apps/bff/src/main.ts` / `server.ts` 只负责启动服务；
  - `apps/agent-cli/src/main.ts` 和 `src/entrypoints/**` 只负责命令入口与运行时装配。
- Web 端新增或调整功能时必须按页面、功能域和通用组件拆分：
  - `src/app/**` 放应用级编排、路由视图状态、导航配置和跨页面类型；
  - `src/pages/**` 放顶层页面；
  - `src/features/<domain>/**` 放领域页面、组件、hooks、领域工具函数；
  - `src/components/**` 放跨领域可复用展示组件或布局组件；
  - `src/lib/**` 放纯函数、格式化、常量等共享工具。
- Web 端禁止把页面、状态编排、领域工具和展示组件继续堆进单个大文件；单个文件超过约 400 行时必须优先评估拆分，入口文件超过约 100 行必须收口。
- BFF 端按业务域组织接口与服务：
  - `src/<domain>/*.controller.ts` 负责对应 REST 入口，controller 保持薄适配；
  - `src/<domain>/*.service.ts` 放业务规则和数据规范化；
  - 通用代理、配置、存储和 HTTP 工具继续放在 `src/` 根部或明确的共享目录；
  - 不得把多个业务域的 CRUD 长期堆在 `AppController` 中。
- Agent 端保持现有领域边界：
  - 命令入口放 `src/entrypoints/**` 或 `src/cli/**`；
  - 运行链路放 `src/runtime/**`；
  - 工具协议与工具实现放 `src/tools/**`；
  - 可复用服务放 `src/services/**`；
  - 新增 agent 能力时优先归入既有领域目录，确实形成新领域时再新增同级目录。
- 跨端新增能力时先确定所属端和领域边界，再落文件；不得为了快速实现把 BFF、Web、Agent 的领域逻辑混在入口文件或通用杂项文件里。
...
- 每次执行完毕后，必须清理由本次执行生成的以 `.` 开头的测试 / 运行产物目录或文件；仅清理可确认是本次运行产生的内容，不删除 `.git/`、`.github/`、`.env*`、`.gitignore`、`.githooks/`、`.gitmessage.txt`、`.npmrc`、`.pnpm*` 等项目配置或版本控制文件。
...
- 禁止提交：
  ...
  - 本地快照；
  - 持久化运行数据。
- 每次完成任务后默认停在待验收状态，不直接执行 `git commit`。
- 用户亲自验收通过后，或用户明确要求提交时，才执行本地 `git commit`；提交前仍需完成清理、状态检查和必要验证。
- 不要执行 `git push`。
