...
#### Scenario: 多入口复用共享 daemon client resolver
- **WHEN** TUI、MCP 或其他前台入口尝试 attach 到本地 daemon
- **THEN** 它们通过统一的 daemon client resolver 探测与初始化共享宿主连接
- **AND** 不在各入口内部分别复制 lock/status/health probe 逻辑
