...
export function parseLayer(raw: unknown): MemoryLayer {
  ...
  if (value === "short_term" || value === "long_term" || value === "both") {
    return value;
  }
  return "both";
}
...
export async function searchMemoryHits(
  ...
  if (layer === "short_term" || layer === "both") {
    ...
    for (const item of items) {
      ...
      hits.push({ ...item, score: scoreEntry(query, item), layer: "short_term" });
    }
  }
  if (layer === "long_term" || layer === "both") {
    ...
    for (const item of items) {
      ...
      hits.push({ ...item, score: scoreEntry(query, item), layer: "long_term" });
    }
  }
