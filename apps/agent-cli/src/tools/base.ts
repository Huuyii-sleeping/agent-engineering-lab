...
const BASE_HANDLERS: Record<string, ToolHandler> = {
  ...
  schedule_pause: async (args) => runSchedulePause(args.id),
  schedule_resume: async (args) => runScheduleResume(args.id),
  schedule_update: async (args) =>
    runScheduleUpdate(args.id, args.prompt, args.cron, args.recurring, args.misfire_policy, args.max_catch_up),
  schedule_stats: async () => runScheduleStats(),
