...
export function SopInspector(props: {
  ...
  else if (props.selectedNode) content = <SopNodeInspector node={props.selectedNode.data.node} availableVariables={props.availableVariables} diagnostics={props.selectedDiagnostics} collapsed={Boolean(props.selectedNode.data.collapsed)} onToggleCollapsed={props.onToggleCollapsed} onChange={(node) => props.onUpdateNode(() => node)} onDelete={props.onDelete} onDuplicate={props.onDuplicate} />;
  ...
  return <aside className="sop-insp">{content}</aside>;
}
