...
export class SkillValidatorService {
  ...
  validatePackage(input: unknown): SkillValidationResult {
    ...
    for (const file of files) {
      ...
      if (file.path.startsWith("scripts/")) {
        errors.push("scripts are not allowed in first-stage custom or remote packages");
      }
  ...
  packageFromRequiredFiles(skillFile: string, metadataFile: string): SkillPackageInput {
    return {
      files: [
        { path: "SKILL.md", content: skillFile },
        { path: "skill.json", content: metadataFile },
      ],
    };
  }
}
