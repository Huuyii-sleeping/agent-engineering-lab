# Workspace Agent 执行规范

## 目标
统一当前工作区的 Agent 执行方式，确保任务推进、测试、提交、归档行为一致且可追溯。

## 语言规范
- 默认使用中文沟通与文档说明。
- 代码、命令、路径、错误栈等技术内容保持原文，不做无意义翻译。

## 任务执行流程（默认）
1. 先阅读对应 `agent_dev/prd/incremental/PRD-XX-*.md`。
2. 按 OpenSpec 流程执行：`new change -> artifacts -> implement -> validate -> archive`。
3. 每个 PRD 完成后先验收，再提交。

## 优先级策略
- P0：优先完善现有功能（当前以 `PRD-13` 为主）。
- P1+：再实现新增功能（`PRD-07` 及以后）。
...
- 允许提交：
  - 源码变更（如 `src/`）
  - 必要文档变更（`agent_dev/prd/`、`openspec/` 等）
- 禁止提交：
  - 临时脚本
  - 本地快照
  - 持久化运行数据
- 每次提交后必须立即执行：`git push`
