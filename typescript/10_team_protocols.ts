...
const SYSTEM = `你是位于 ${WORKDIR} 的团队负责人。请使用关停与计划审批协议管理队友。`;
...
class TeammateManager {
  ...
  private async teammateLoop(name: string, role: string, prompt: string): Promise<void> {
    const sysPrompt = `你是 '${name}'，角色为 ${role}，位于 ${WORKDIR}。在执行重大工作前请通过 plan_approval 提交计划；收到 shutdown_request 时请通过 shutdown_response 回复。`;
    ...
    for (let i = 0; i < 50; i += 1) {
      ...
      for (const toolCall of toolCalls) {
        ...
        const output = await this.teammateExec(name, toolCall.function.name, args);
